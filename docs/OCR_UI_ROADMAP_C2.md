...
| 2026-09-23 | B13 | **crítico, ciclo 2: a calha da página vale sem prosa, e só uma lista de lances a cruza; notas de variantes são texto corrido; prosa por palavras pede continuação** (`rows._page_gutters`, `page_band_share`, `_numbers_column`, `_move_column`, `_notes`, `prose_continued`) | O crítico reprovou com um índice real sem prosa: a Karpov 2 p. 268 (duas colunas de nomes e páginas) intercalada verbete a verbete pelo importador, CER 0,76 contra a ordem do livro -- a regra do ciclo 2 descartava a calha única que ela mesma achava quando não havia prosa de nenhum lado. Uma calha única com duas faixas de largura comparável é fronteira, prosa ou não; só uma lista de lances a cruza, por evidência positiva (números à esquerda, um lance por linha à direita -- as listas do Dvoretsky seguem por linha); notas com números de lance dentro da linha nunca entram; a prosa por palavras pede linhas que continuam a frase (a tabela de aberturas por extenso, 0,4646 → 0,0113). 280 páginas reais varridas: 43 mudam (46 no ciclo 2), a Karpov 2 p. 268 idêntica ao desligado | construtor + crítico |
| 2026-09-23 | C18 | **crítico, ciclo 2: as quatro zonas do rodapé à vista** (tronco `rodape.LARGURA_DA_MENSAGEM = 320`, `LARGURA_DA_ZONA = 240`, `RotuloElidido(piso=...)`; suíte: o portão mede as quatro zonas, sabotagem `reserva`; as ações da Revisão de texto fora da rolagem) | A reserva de 480 px da mensagem saía das zonas de dispositivos e de ocupação -- 0 px, cortadas em 46 dos 46 livros a 1248 px, a queda para a CPU em silêncio --, e o portão passava porque não as media. A reserva é a frase até 320 px, as duas zonas têm o texto inteiro até 240 px, cede o nome do livro (elidido no meio); o piso é do rótulo, medido com a fonte de agora. O portão mede as quatro zonas com o nome mais longo e um comum; a sabotagem do rodapé do ciclo 2 reprova. A 1280×641 as ações da Revisão de texto ficam à vista (a fileira de figurinas desce abaixo da dobra, dito) | construtor + crítico |
| 2026-09-23 | A15 | **crítico, ciclo 2: mais três brechas do `test_strings`, e a evidência que se refaz** | `", ".join(D)` e `[*D]` enumeram; a lista minúscula passada direto a uma chamada que escreve na tela é varrida. A evidência do A/B guarda o `answered` de cada item (a média de cada estrato se refaz dos itens) e o `bench_sol` grava o código não commitado que mediu (`dirty_code`): as corridas do ciclo 2 gravavam 99546e9 e tinham medido a árvore commitada depois como 4fc0f4d | construtor + crítico |
| 2026-09-23 | B13 | **crítico, ciclo 3: texto corrido sem contar palavras; de duas calhas, a vizinha da prosa; a avaliação separada é parte do lance; o filtro da semente com teste** (`rows._running`, `running_lines`, `_continues`, `_page_gutters`, `_move_tokens`, `_seeds`) | O crítico reprovou com a página que construiu na medida da Gallagher p. 50: notas comuns (três palavras por linha, poucos lances dentro) ao lado da lista de lances sem tabulação eram juntadas linha a linha, e o importador **aceitava** a página (0,0052 → 0,6440). Um bloco de 4 linhas ou mais, com a mediana acima de uma célula e um terço das linhas abrindo com palavra minúscula (um lance não é palavra), é texto corrido; de duas calhas ou mais (a Gallagher real: a da página e a tabulação das pretas), a da página é a vizinha da prosa; «31 Nh1 !?» é um lance. As páginas do crítico voltam ao desligado; a Karpov 2, a Gallagher (0,2028) e as tabelas do ciclo 2 ficam como estavam; a tabela «nome | período» segue por colunas, dito; as 280 páginas e a Estrin inteira com a leitura ligada igual à do ciclo 3 | construtor + crítico |
| 2026-09-23 | C18 | **crítico, ciclo 3: a ordem do Tab da Revisão de texto de volta à visual, o custo do rodapé dito, e o arnês que mede a linha dele** | Tirar as ações da rolagem pôs as seis antes do cartão na cadeia do foco; a cadeia é posta à mão, com teste. A reserva de 320 px (~57 caracteres) elide a frase de erro do próprio produto sem o `.pt` em 46 de 46 livros até 1440 px, e a ocupação com o aquecimento do modelo deixa o nome do livro com 95 px -- ditos, nas docstrings do tronco (`7cd400a`) e no relatório. O arnês do portão media às vezes as zonas reescritas pelo trabalho do próprio produto (a leitura do Dataset, os dispositivos): põe a linha de novo, mede quando ela assenta, e grava o commit e o código fora dele | construtor + crítico |
| 2026-09-23 | A15 | **crítico, ciclo 3: as oito brechas novas do `test_strings` ditas, não fechadas** | A varredura julga cada literal pela posição dele; o crítico construiu oito formas de um texto de tela chegar à tela por nomes, chamadas, atributos e módulos. Seguir o dado é outra ferramenta (análise de fluxo), não mais uma regra; a varredura do crítico sobre o código real não acha frase de tela escondida hoje | construtor + crítico |
| 2026-09-23 | B13 | **crítico, ciclo 4: a calha antes de cada coluna de índice, a calha entre uma partida e o texto, e a concordância na ordem de cada motor** (`rows.index_column`, `_index_gutters`, `_game_gutters`, `list_min_heads`, `game_min_lines`; `arbiter._engine_order`) | O crítico reprovou com índices de duas calhas ou mais e nenhuma prosa, intercalados verbete a verbete: a Flores pp. 460–464 (raster, do acervo; duas páginas **aceitas** com CER ~0,7), a Yusupov 4 p. 206 e a Nunn p. 288 digitalizadas, três colunas compostas aceitas trocadas. Uma coluna cujas cabeças, na margem dela, sobem em ordem alfabética é uma lista, e a calha antes dela é da página; a calha entre uma partida e uma coluna de texto também, procurada na faixa ocupada mais perto (o rio de uma coluna justificada) -- o conserto da franja do texto corrido (0 de 41 páginas em risco embaralhadas; o ciclo 4, 24). Na Nunn p. 288 o B13 lia o Tesseract certo e o árbitro escolhia o RapidOCR intercalado: a concordância comparava as linhas já movidas; agora cada leitura é comparada na ordem do seu motor, e o B13 não decide quem ganha (0,3212 → 0,0193). A concordância nos dois sentidos do `difflib` foi medida e descartada: punha o RapidOCR intercalado na Karpov 2 pp. 267/272 e na Vladimirov pp. 380/384/385. As 280 páginas do crítico, a Estrin, a Gunderam e a Kmoch com a leitura desligada igual à do ciclo 4 e a ligada mudando em 6 páginas, todas para melhor; os 747 itens da fila de A/B iguais, salvo a confiança relatada de um | construtor + crítico |
| 2026-09-23 | C18 | **crítico, ciclo 4: o teclado com a tecla, e não com a cadeia** (o campo da verdade do cartão deixa o Tab sair; o cartão rola até o foco que entra de fora; o portão `teclado` aperta a tecla nos dois sentidos e mede o foco à vista; tronco `37f662b`, `4f3e218` e `7a78367`: o comentário, a lista de lances e a caixa de colar do Estudo; a Rotulagem) | O conserto do ciclo 3 existia só na cadeia do foco: com a tecla, o Tab parava na verdade da linha escrevendo tabulações, as ações fora de alcance, e o Shift+Tab das ações punha o foco numa figurina abaixo da dobra (0×0 px). Teste da tecla da tabela à última ação e de volta, cada controle à vista, com as sabotagens; o portão diz onde a tecla empaca e quem recebe o foco fora da vista -- achou o comentário, a lista de lances e a caixa de colar do Estudo e a «Leitura do motor» da Rotulagem (consertados) e a «Folha transcrita» do Texto (arquivo de outra sessão: REPROVADO, dito). O arnês da janela espera a linha nas duas passadas, e a sabotagem nova `linha` achou que «medido na linha do arnês» mentia quando o produto reescrevia sem parar. No commit, as oito áreas e os catorze diálogos andam com a tecla nos dois sentidos com o foco à vista, menos o Texto | construtor + crítico |
| 2026-09-23 | Sol | **crítico, ciclo 4: o motor que falha é dito na página** (`OcrEngineBase.recognize` marca `failed`; `OcrService._note_engine_failures`) | A Stean p. 165 piorou sem aviso com a máquina sem memória: o RapidOCR roda no processo, o `onnxruntime` levanta na alocação, a leitura dele sai vazia com o aviso e o árbitro fica com a do Tesseract (a decisão é revisão). Agora a página diz qual motor falhou e por quê; um `MemoryError` o motor não pega, e a página não sai (dito) | construtor + crítico |
| 2026-09-23 | B13 | **crítico, ciclo 5: uma lista ao lado de outra é lista, em ordem ou não, curta ou longa, e uma partida ao lado de uma tabela é uma partida e uma tabela** (`rows._row_kind`, `_line_kinds`, `_band_form`, `_list_gutters`, `_names`, `mixes`; a regra alfabética sai) | O crítico compôs páginas que a regra alfabética não via: «jogador / adversários» pela classificação (0,0000 → 0,6577, **aceito**), a última página de um índice com cinco verbetes (0,6634 → 0,3196, aceito), uma partida ao lado da classificação (0,2218 → 0,7564); e ela errava do outro lado, com uma tabela cujos nomes e países subiam em ordem por acaso. A forma de cada faixa, linha a linha (verbetes, nomes com valores, nomes, números, pontos), e a calha entre duas listas da mesma forma é da página; um grupo nunca tem uma partida e uma coluna de nomes. Medido nas 1.622 leituras que passaram pelo B13 nas medidas do ciclo 5, guardadas e reordenadas pelas duas regras: 8 mudam, nenhuma para pior; nenhum dos 747 itens do corpus, nenhuma página que o crítico mandou manter. E no `7f839f0`, a célula de uma linha tem a altura de uma linha (`_spans_rows`, `_one_row_high`): a coluna de colocações que o Tesseract lê como uma linha só, oito linhas de alta, se juntava à partida e puxava o quinto lance para a segunda linha (a P|Tn do crítico, 0,3376 → 0,2707; 26 das 1.622 leituras mudam, nenhuma do corpus, e a Gallagher p. 175 volta à ordem do livro). Fica, dito: a classificação em duas metades com os pontos perdidos pelo OCR | construtor + crítico |
| 2026-09-23 | C18 | **crítico, ciclo 5: o teclado onde o usuário o usa** (a tabela da Rotulagem deixa o Tab sair e as setas guardam o foco nela; `foco_a_vista` na suíte e no tronco `6e68315` — os modos da aba Livro, a lateral da Galeria, a lista de bases —, pela troca de foco da aplicação e com o controle inteiro; o portão com as rolagens no topo em cada sentido, a Rotulagem com uma página reconhecida — e, no `99ad8d7`, o JSON com as linhas da tabela na medida, que o `2c81e36` gravava `null` —, as sabotagens `foco` e `tabela`) | O portão dizia PASSOU com a Rotulagem de tabela vazia e o Shift+Tab logo depois do Tab, com as rolagens descidas: com uma página de linhas a tecla não saía da tabela, e com a rolagem no topo o foco ficava a 0 px no Resultado, na Galeria, no Estudo e na Revisão. As duas sabotagens reprovam onde o crítico achou os defeitos | construtor + crítico |
| 2026-09-23 | Sol | **crítico, ciclo 5: a falha do motor chega a quem revisa** (`OcrService._with_failures`, `RegionRecognition.engine_failures`; `PdfImporter._failed_for_review`) | A falha era dita só no traço: a região em que um motor falhou leva a falha entre os motivos, a região aceita com um motor a menos vai para revisão, e a página cujo OCR levanta entra na fila de revisão, abstida, com o motivo | construtor + crítico |
