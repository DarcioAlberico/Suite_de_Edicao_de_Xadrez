...
>
> **Ciclo 6 do crítico** (§0.8). O ciclo 5 reprovou com dois bloqueantes (o B13 ainda intercalando
> duas listas lado a lado que a regra alfabética não via como índice — fora de ordem ou curtas,
> **aceitas** intercaladas — e juntando uma partida à tabela ao lado dela; e o teclado falhando
> onde o portão dizia PASSOU: a Rotulagem com uma página de linhas prendia o Tab, e o foco que
> entra nas rolagens do tronco ficava a 0 px com a rolagem no topo); os consertos estão no tronco
> `6e68315` e na suíte `2c81e36` (código, testes, a transcrição do ciclo 5) e `7f839f0` (a linha
> alta que o construtor achou numa página do crítico, §0.8 item 1.4) e `99ad8d7` (o portão do
...
- **A 1280×641 as ações do Resultado ficam abaixo da dobra** (não bloqueante 6 do ciclo 3, igual
  ...
  1 na Clássica —, alcançáveis pela barra do modo (`…\_critico_f5\c18_visiveis.py`) e, desde o
  ciclo 6, pela tecla: a rolagem mostra inteiro o controle que recebe o foco, venha ele de onde
  vier (§0.8 item 2).
...
- **A última página de um índice de três colunas, lida junta pelo próprio Tesseract** (não
  ...
  um à linha da segunda («Ivkov 268 Robatsch 178») — ligado igual ao desligado, **aceito** (0,0931 e
  0,1544 pelo importador, `c5taque\indices_ataque.pdf` pp. 1–2). O B13 não forma grupo: é a
  segmentação do Tesseract, e a calha que a página tem não aparece na leitura dele. Dito, não
  resolvido.
...
- **Um `MemoryError` dentro de um motor derruba a página**: o `OcrEngineBase.recognize` pega as falhas de operação (`OSError`, `ValueError`, `RuntimeError`) — e agora a página as diz —, mas não o `MemoryError` (simulado no RapidOCR, `scratchpad/c5/stean165_varredura.py`: a página não sai). A leitura dela continua sem sair; desde o ciclo 6 a página entra na fila de revisão, abstida, com o motivo, e a região em que um motor falhou leva a falha entre os motivos (§0.8 item 3(e)).
...
- **A classificação de um torneio em duas metades** (ciclo 6, uma página que o construtor compôs
  para quebrar a regra das listas, `scratchpad/c6/paginas_c6.py`): «Aljechin 7½ | Lilienthal 3½»
...
- **Uma tabela só de nomes é lida como listas**: em duas colunas (um emparceiramento «Brancas |
...
- **Um ponto inteiro sozinho numa linha («7») é tomado por número de lance**, e um grupo só aceita
  ...
  classificação).
- **Pelo serviço, na página do PDF, a Nunn p. 288 sai com a leitura do RapidOCR**, intercalada:
  0,7516 ligada e desligada (a varredura do ciclo 5 e a sonda do crítico); pelo importador, a mesma
  página digitalizada sai 0,0193. O B13 não decide quem ganha ali (a concordância na ordem do
  motor, ciclo 5); decide o caminho, e o conserto que falta é o do detector do RapidOCR, que junta
  caixas através da calha.
...
- **1. O B13 intercalava as colunas de um índice quando a página tinha duas calhas ou mais**
  ...
  **Remedido** (as sondas do crítico com o código final; `scratchpad/c5/sondas3.sh`, importador
  ...
  `accepted`; a 463 na ordem do livro, e a 461 com as páginas de três verbetes fora do lugar — o
  que este parágrafo dizia, «agora na ordem do livro», valia para a 463 só; §0.8 item 3(d)); a Yusupov 4 p. 206 0,0465 = desligado; a Nunn p. 288 0,3212 →
...
- **2. O teclado com a tecla, e não com a cadeia** (bloqueante 2). O conserto do ciclo 4 existia só
  ...
  `test_rotulagem_view.py`). No commit (`b6978ec`, tronco `7a78367`), o portão do teclado nas três peles e nos três tamanhos: as oito áreas e os catorze diálogos andam com a tecla nos dois sentidos, com o foco à vista, menos a «Folha transcrita» do Texto — REPROVADO, dito. (O portão media a Rotulagem com a tabela vazia, e o Shift+Tab logo depois do Tab, com as rolagens já descidas por ele; o crítico, no ciclo 5, mediu a Rotulagem com uma página de linhas e cada sentido com as rolagens no topo, e ela e quatro áreas do tronco falhavam — §0.8 item 2.)
- **3. Os itens do mesmo ciclo.** (a) **A franja do texto corrido, com os números** (não
  ...
  **0 das 29** — e 0 das 41 com as 11 páginas do índice da Dvoretsky e a da Aagaard (o ciclo 4, 23
  das 41 — o ciclo 5 escreveu 24, e a saída que ele mesmo cita soma 23; §0.8 item 3(c); `scratchpad/c5/sondas3/notas_risco_w24.out`, a cópia do `b13_notas_medida.py` do crítico
...
- **O que fica, dito** (§0.3): a «Folha transcrita» do Texto empaca a tecla (arquivo de outra
  sessão); o RapidOCR que intercala as colunas de uma página cujo detector junta caixas através da calha (a razão por que a concordância nos dois sentidos foi descartada); a guarda de figurinas que tira o RapidOCR da Stean p. 165 pelo importador; o `MemoryError` que derruba a página; e, dos ciclos anteriores, as oito brechas do A15, a Estrin pp. 29/77 fora do grupo, o custo da reserva de 320 px e o nome a ~95 px, as ações do Resultado abaixo da dobra a 1280×641 (a tecla chega a elas; «e as mostra» era falso com a rolagem no topo — 0 px, o crítico no ciclo 5 — e é verdade desde o ciclo 6, §0.8 item 2), o portão só `offscreen`.

### 0.8 O ciclo 5 do crítico: REPROVADO com dois bloqueantes, e o que mudou

O crítico conferiu os dois bloqueantes do ciclo 4 nas páginas que ele tinha dado e os deu por
resolvidos ali (a Flores pp. 460–464, o `tres.pdf`, a Nunn p. 288 e a Yusupov p. 206 pelo
importador, tudo o que ele mandou manter; o cartão da Revisão de texto com a tecla nos dois sentidos),
mas não na classe, e reprovou por dois defeitos novos (veredito em `OCR_UI_ANALISE_C2_CRITICAS.md`
«Fase 5», ciclo 5). O construtor não contestou nenhum item.

- **1. Uma lista ao lado de outra é lista, em ordem alfabética ou não, curta ou longa** (bloqueante
  1). A regra do ciclo 5 não era a classe que o crítico tinha pedido: ela chamava de índice só a
  coluna cujas cabeças sobem em ordem alfabética, seis ao menos. O crítico compôs páginas como um
  livro digitalizado (Times 9 pt, 300 DPI) e, pelo importador de produção, três execuções iguais byte
  a byte: «jogador / adversários» em três colunas, os jogadores pela classificação (o formato da
  Yusupov p. 206 sem a ordem), **0,0000 aceito → 0,6577 aceito** («Carlsen Caruana 82
  Nepomniachtchi 127»); a última página de um índice «nome | páginas» (o formato da Nunn p. 288) com
  cinco verbetes na segunda coluna, 0,6634 → **0,3196 aceito** («Aaron 249 Ivkov 268»); e, pelo
  serviço, uma partida ao lado da classificação do torneio, 0,2218 → **0,7564** («1 e4 e5 Aljechin
  7½»). E a regra alfabética errava do outro lado também: a tabela de um torneio com os nomes e os
  países em ordem por acaso saía como dois índices, 0,7509 → 0,6367 (não bloqueante 11). O que
  mudou (`rows.py`; `test_table_rows.py` +6, `test_listas_compostas.py`, novo, e
  `test_indices_do_acervo.py` com a sabotagem nova):
  1. **a forma de uma faixa, linha a linha** (`_row_kind`, `_line_kinds`, `_band_form`). Cada
     linha diz o que tem: só números (`231, 246`); um nome e os números dele (`Aaron 249`, `Van
     den Ende M60`, `Polugaevsky — 999, 1046`, `Carlsen – Caruana 1–0 45`: a página ou o número do
     exercício, com a letra da Nunn antes e o travessão entre o nome e eles; o resultado de uma
     partida é parte do nome); um nome e um valor (`Aljechin 7½`); só pontos; só nomes; ou nada
     disso. Um nome e as páginas dele que o Tesseract leu como duas linhas na mesma altura (a
     segunda coluna da Flores p. 460, «Berzinsh» e «436, 459») contam como um verbete. A faixa é
     uma **lista de verbetes** quando um terço das linhas ou mais é um nome com os números dele,
     quase todas são nomes, verbetes ou números (as cabeças e os adversários da «jogador /
     adversários», as linhas de continuação de um verbete longo), e os números são uma lista em
     algum lugar ou curtos — `999, 1046` é um índice (a Karpov 1 p. 398, com números de exercício de
     quatro algarismos: a primeira versão desta regra os tomou por anos e perdeu a calha), e uma
     coluna de `Moscovo 1985` é a coluna de uma tabela;
  2. **a calha entre duas listas da mesma forma é da página** (`_list_gutters`), qualquer que seja
     a ordem e o tamanho: uma faixa de nomes seguida da faixa das páginas deles (a Nunn: «Aaron |
     249») é uma unidade de verbetes, uma faixa de verbetes (`Aaron 249`) é outra, uma faixa de
     nomes seguida da dos pontos é uma unidade de nomes com valores; entre duas unidades seguidas da
     mesma forma, a calha é da página. E numa página só de nomes, em colunas, toda calha entre elas
     (a primeira versão desta regra intercalava uma lista de nomes em três colunas, em ordem, que a
  ...
  3. **uma partida nunca se junta a uma coluna de nomes** (`_moveish`, `_names`, `mixes` em
     ...
     «Hc?», «4 \a3») e o «draw.» de um parágrafo caído sobre os números dos lances não são nomes: a
     primeira versão chamava de células tudo o que não era lance e tirava as brancas da lista da
     Gallagher p. 50 — achado nas leituras guardadas antes do commit, e é o que a segunda desfaz;
  4. **a célula de uma linha tem a altura de uma linha** (`_spans_rows`, `_one_row_high`; o segundo
     commit de código do ciclo, **`7f839f0`**). Na P|Tn do crítico — a partida ao lado da
     classificação com as colocações, «1. Aljechin 7½» — o Tesseract lê a coluna «1.» … «12.» como
     uma linha só, de 378 px de altura, «SANDMNS WNP». O `2c81e36` a juntava à partida (uma linha ao
     lado de toda linha, e não uma coluna de nomes), e a linha do `_rows` que a recebia tomava a
     altura dela e o quinto lance: «2 Nf3 Nc6 **5 O-O Be7** 2. WNP», 0,3376 contra 0,3185
     desligado. A sonda do crítico só marca «embaralha» acima de 0,05 do desligado; o construtor
     achou o defeito lendo o texto dela no `2c81e36` (`scratchpad/c6/ptn_texto.py` →
     `ptn_texto.out`). Um bloco com uma linha na altura de duas linhas de outro, linhas que não
     estão numa altura só entre si, não entra no grupo dele; duas linhas na mesma altura são uma
     linha da tabela (um nome e as páginas dele que o Tesseract leu como duas linhas, a Flores p.
     460). Medido nas mesmas 1.622 leituras, a regra do `2c81e36` × a do `7f839f0`
     (`todas_leituras2.py` → `todas_leituras_c6c.out`; `diff_leituras.py` →
     `diff_leituras_c6b.out`): **26 mudam**, nenhuma das 1.048 do corpus, nenhuma das páginas que o
     crítico mandou manter. 20 são a leitura escolhida da página: a P|Tn **0,3376 → 0,2707**; a
     Gallagher p. 175, a única página que o `2c81e36` piorava, volta à ordem do livro («4 Nc3 Nf6»
     depois de «3 Nf3 d6»), igual à leitura desligada; e 18 páginas de ruído de diagrama (a
     Aagaard *Excelling* pp. 180, 184, 185, 186, 191 e 192; a Estrin pp. 61, 72 e 77; a Gallagher
     pp. 17, 33, 95, 109, 132, 137, 138 e 178; a Stean p. 154). As outras 6 são leituras que o árbitro não escolhe (a
     Reinfeld 1977 p. 319, a Gunderam p. 10, a Kmoch p. 18, a Flores p. 465, a Estrin p. 64 e a
     Gallagher p. 128, esta com a coluna partida pelo Tesseract em dois blocos intercalados, que a
     regra do `2c81e36` punha na ordem por acaso, com o ruído do diagrama de ponte entre eles); as
     seis páginas rodaram de novo pelo serviço nos dois commits (`scratchpad/c6/varre_c7.sh` →
     `varre_c7/`, `cmp_varre_c7.py` → `varre_c7/cmp.out`): cinco saem idênticas nos dois, e iguais à
     leitura desligada; a Reinfeld p. 319 muda, **para melhor** — os consertos de palavra do serviço,
     que alinham as leituras dos motores depois do B13, punham no `2c81e36` palavras de outra linha
     em dois títulos («14. Ataque «rayos la primera», «18. «Zugzwang» de Dama»), e no `7f839f0` os
     dois saem como na página («rayos X»», «Zugzwang»»); a segunda linha do sumário ainda junta
     dois títulos, e as páginas 5 e 7 continuam perdidas pelo OCR. Testes: a partida ao
     lado das colocações lidas numa linha alta, com a sabotagem executada no teste (sem a regra, o
     quinto lance sobe para a segunda linha), e duas linhas na mesma altura, uma linha da tabela
     (`test_table_rows.py` +2).

  **Como foi medido antes de rodar o OCR de novo.** Toda leitura que passou pelo B13 nas medidas
  do ciclo 5 — as sondas do crítico pelo importador, as páginas compostas dele pelo serviço, as 17
  varreduras (a Gallagher inteira, as páginas finais de nove livros, a Estrin, a Gunderam e a
  Kmoch, os índices reais) e os 747 itens do corpus — foi guardada antes do B13
  (`scratchpad/c6/cache_b13.py`, `cache_corpus.py`, `cache_todos.sh`; o `src` do `4e42158`) e
  reordenada pelas duas regras, a do ciclo 5 (`4e42158`) e a do `2c81e36` (`b13_formas.py`,
  `todas_leituras.py`): **1.622
  leituras, 8 mudam** — as duas páginas do crítico (p. 4 **0,3148 → 0,0000**, p. 8 **0,6577 →
  0,0000**, no nível do B13, antes dos consertos de palavra do serviço), três das cinco páginas de
  partida × tabela (**0,7600 → 0,1382**, 0,7420 → 0,3376 — acima do desligado, 0,3185: o item 4,
  e 0,2707 no `7f839f0` —, 0,7580 → 0,2675; a T|P, que o crítico
  não deu por embaralhada, 0,2073 → 0,2036), a tabela com as duas colunas em ordem (lida por linhas:
  «Alekhine Argentina 2690 8½»), a Stefaniu p. 245 (o colofão: o preço e o ISBN, que o B13 do
  ciclo 5 juntava numa linha, cada um na sua) e uma leitura da Flores p. 462 que o árbitro não
  escolhe (a regra do ciclo 5 a intercalava, 0,7167; a do `2c81e36` a deixa por colunas, 0,2379). Nenhum dos
  747 itens do corpus muda, nenhuma página que o crítico mandou manter muda (a Flores pp. 460–464,
  a Nunn pp. 288–289, a Yusupov p. 206, a Aagaard pp. 892–893, o `tres.pdf`, o `construido.pdf`, a
  Karpov 2 p. 268, a Gallagher pp. 50–53, as notas inglesas), e nenhuma das ~450 páginas das
  varreduras muda além da Stefaniu p. 245 (`scratchpad/c6/cmp_rows_final_caches.out`,
  `todas_leituras.out`). Uma leitura não escolhida que muda pode mudar o escolhido: as duas páginas
  das varreduras onde isso acontece rodaram de novo pelo serviço, com o código do `2c81e36` — a Flores
  p. 462 sai igual ao ciclo 5 (0,0224), a Stefaniu p. 245 só com o colofão (`c6/varre_c6/`).
  **As sondas do crítico no commit** (`scratchpad/c6/sondas_c7.sh` → `c6/sondas7/`, no `7f839f0`;
  o OCR do `99ad8d7` é o mesmo — ele muda só o portão do teclado): pelo importador, comparadas
  região a região com as dele do ciclo 5 (`cmp_sondas_c7.py` → `sondas7/cmp_sondas.out`), o
  `indices_ataque.pdf` p. 8 sai **igual ao desligado** (0,0000) e a p. 4 **0,0048**, aceita, sem
  linha que junte dois verbetes (`cer_pdf.py` → `sondas7/cer_sondas.out`); as outras sete páginas
  dele, o `tres.pdf`, o `indices_raster.pdf`, a Flores pp. 460–464, as notas inglesas, o
  `construido.pdf`, a Karpov 2 p. 268 e a Gallagher pp. 50–53 saem **idênticas às do ciclo 5
  dele**, ligadas e desligadas — o que ele mandou manter não mudou —, e idênticas às do `2c81e36`.
  Pelo serviço, as cinco páginas de partida × tabela saem no desligado ou abaixo (P|T 0,2218 →
  **0,1418**, P|Tn 0,3185 → 0,2611, T|P 0,2182 → 0,2109, Pt|T 0,5055 → 0,1527, P|T3 0,3185 →
  0,2611), a tabela em ordem 0,7509 → 0,1280 e fora de ordem 0,7448 → 0,1207; as tabelas do ciclo 3
  dele, as estreitas, a tabulada, a lista recusada, as do ciclo 2 e as notas de risco com as linhas
  de resumo iguais às do ciclo 5 dele, menos uma: a «classificação do torneio (1, 2, 3…)» a 150 DPI,
  0,1818 → **0,1866** (desligada 0,4880) — a coluna de colocações, que o Tesseract lê como uma linha
  alta («ta, de) Ww mu»), sai numa linha antes da tabela, e não mais colada à linha 2; as linhas da
  tabela saem limpas e «Ding, Liren» certo, e o CER sobe pela posição do lixo; e a franja do texto
  corrido ao espelho, **0 de 41** nos quatro leiautes (`sondas7/franja_espelho.out`): a Dvoretsky p. 789 N|Pt, a única que o ciclo 5 embaralhava (0,5052 → 0,6522, o não bloqueante 9 dele), sai **0,3720**, abaixo do desligado.

  **As minhas tentativas de quebrar a regra, antes do crítico** (`scratchpad/c6/paginas_c6.py`, no
  molde do gerador dele; no nível do B13, `pontua_paginas.py`): uma lista só de nomes em três
  colunas, em ordem — 0,0000, igual ao desligado; fora de ordem, em duas e em três colunas — o
  próprio Tesseract junta as colunas numa linha (0,7591 e 0,8007 desligado), e o B13 não forma grupo;
  a última página de um índice com **dois** verbetes na segunda coluna — 0,6898 desligado → 0,1390;
  três colunas com um verbete só na terceira — 0,0370, igual ao desligado; uma partida ao lado de um
  índice — 0,0149, igual; um emparceiramento «Brancas | Pretas» (uma tabela só de nomes) — lido por
  colunas, 0,6853, como antes (§0.3). E uma que quebra: **a classificação de um torneio em duas
  metades**, «Aljechin 7½ | Lilienthal 3½», 0,3316 desligado → **0,7053** ligado, igual à regra do
  ciclo 5 (e igual no `7f839f0`) — o OCR perde os pontos inteiros (os «7», «6» sozinhos) e lê «7½» como «Th» e «6½» como
  «6%», e a segunda metade não tem pontos lidos: nada diz que a segunda coluna de nomes continua a
  primeira (§0.3).

- **2. O teclado onde o usuário o usa: a Rotulagem com uma página de linhas, e o foco que entra
  nas rolagens** (bloqueante 2). O crítico mediu a Rotulagem com a Gallagher p. 51 reconhecida (66
  linhas): a tabela «Linhas da página» andava com o Tab de célula em célula (o padrão do Qt), e cada
  troca de linha mandava o foco à «Verdade da linha» — a tecla não saía do laço, e de volta o
  Shift+Tab alcançava 23 dos 37 controles. E, com as rolagens no topo, como quem chega à área as
  encontra, o Shift+Tab punha o foco a 0x0 px em «Esconder incerteza» (Resultado) e «Copiar
  cabeçalhos para todos» (Galeria) nas três peles, e em «Comentário do lance» e «Próximo pendente»
  na Fita. O portão não via nenhum dos dois: media a Rotulagem com a tabela vazia, e a volta do
  Shift+Tab corria logo depois da do Tab, com as rolagens já descidas por ela. O que mudou:
  1. **a Rotulagem com linhas**: a tabela deixa o Tab sair (`setTabKeyNavigation(False)`), as
     setas andam pelas linhas com o foco na tabela e o cartão acompanhando (`pelas_setas`:
     `mostrar_verdade(focar=False)` quando a linha mudou pela tecla com o foco na tabela), e o
     Enter leva ao campo da verdade (`activated`); quem escolhe a linha com o mouse, ou acabou de
     decidir uma, continua escrevendo em seguida. O mesmo na Revisão de texto, onde o comentário
     dizia «as setas andam pelas linhas» e a primeira seta mandava o foco à verdade (não bloqueante
     5). Testes: a Rotulagem com uma página reconhecida pelo próprio painel (12 linhas) anda nos
     dois sentidos pelas 36 paradas com cada controle à vista, as rolagens no topo antes de cada
     sentido — a sabotagem (a tabela com o Tab de volta) reprova; as setas nas duas abas guardam o
     foco na tabela e o Enter o leva à verdade — a sabotagem (o cartão tomando o foco a cada linha)
     reprova (`test_rotulagem_view.py`, `test_revisao_de_texto_view.py`);
  2. **toda rolagem que o Tab alcança de fora mostra o controle que recebe o foco, inteiro**: no
     tronco (`6e68315`), `qt/foco_a_vista.py` nos modos da aba Livro (`PainelPrincipal._rolagem_de`),
     na lateral da Galeria e na lista do diálogo «Base de partidas»; na suíte, o
     `ui/widgets/foco_a_vista.py` refeito. Os dois ouvem a troca de foco da aplicação
     (`QApplication.focusChanged`), e não um filtro posto nos controles de quando a rolagem nasceu
     (a lista de bases se redesenha); e rolam até o retângulo do controle, e não até a `microFocus`
     de um campo de texto (`ensureWidgetVisible`), que não rolava nada quando o cursor já estava à
     vista com o resto do campo cortado — a «Leitura do motor» da Fita com 28 de 48 px (não
     bloqueante 7). Testes: no tronco, o seguidor numa janela curta (o foco de fora no topo e com a
     caixa meio à vista, um controle criado depois) e as três rolagens do produto com a tecla de
     verdade e a rolagem no topo, cada uma com a sabotagem (`tests/test_qt_foco_a_vista.py`, 6); na
     suíte, `test_foco_a_vista.py` (o controle meio à vista: 20 de 48 px com o filtro do ciclo 5,
     `scratchpad/c6/probe_antigo.py`);
  3. **o portão mede como o usuário encontra a janela** (`ui/audit/teclado.py`): cada volta da
     tecla começa com toda `QScrollArea` no topo (`_rolagens_no_topo`); a Rotulagem é medida com
     uma página do livro do portão reconhecida pelo serviço do produto (o Kemeri p. 6, 47 linhas),
     num projeto temporário — o `labeling/` de quem roda o portão não é tocado
     (`_rotulagem_com_uma_pagina`) —, e o JSON grava as linhas da tabela no momento da medida (no
     `2c81e36` ele gravava `null`: o `setdefault` em toda área guardava o `None` da primeira, o
     Resultado — achado pelo construtor ao ler o JSON da corrida, e consertado no **`99ad8d7`**,
     que também reprova a passada em que a página se reconheceu e a tabela não estava cheia na
     medida; teste com a sabotagem, o registro antigo, `test_rotulagem_view.py`); o
     diálogo «Base de partidas» é medido também com 40 bases (a lista rola); e duas sabotagens,
     `--sabotar foco` (todo seguidor desligado, suíte e tronco) e `--sabotar tabela` (a tabela da
     Rotulagem com o Tab de volta). O teste do instrumento: a volta antiga (depois da do Tab, sem
     voltar ao topo) não vê a figurina escondida, a nova vê (`test_teclado_tecla.py`);
  4. as frases que o crítico corrigiu: a do §0 e a do §C18 («cada controle à vista» medido com a
     tabela vazia), a do §0.7 («a tecla chega a elas e as mostra»: com a rolagem no topo, 0 px — é
     verdade agora), o roadmap e o `HANDOFF.md`.
  **No commit** (`99ad8d7`, tronco `6e68315`; `scratchpad/c6/janela_c7.sh` → `c6/janela7/`, `resumo_portoes.py` → `janela7/resumo_teclado.out`): o teclado com a tecla nas três peles, a 1280×800, 1248×640 e 1280×641, cada sentido com as rolagens no topo, a Rotulagem com a p. 6 do Kemeri reconhecida — **47 linhas na tabela na medida**, que o JSON agora grava — e o diálogo de bases também com 40: as sete áreas da suíte e do tronco menos o Texto e os quinze diálogos andam nos dois sentidos com o foco à vista (na Foco a 1280×641 a tecla alcança 31 controles no Resultado, 59 no Estudo, 38 na Revisão, 27 no Dataset, 53 na Galeria, 63 na Rotulagem e 49 na Revisão de texto), e a «Folha transcrita» do Texto empaca — REPROVOU, arquivo de outra sessão. As sabotagens a 1280×641 REPROVAM nas três peles: `foco` com o foco fora da vista no Resultado («Esconder incerteza»), na Galeria («Copiar cabeçalhos para todos»), na Rotulagem («Leitura do motor», «Linhas da página»), na Revisão de texto («Letras → figurinas») e na lista de 40 bases («base_39.pgn») — na Fita também no Estudo («Comentário do lance») e na Revisão («Próximo pendente») —, e `tabela` com a Rotulagem empacando em «Linhas da página» nos dois sentidos. O `labeling/project.json` de quem roda o portão continua o de 2026-09-15.

- **3. Os itens do mesmo ciclo.** (a) **As páginas que o B13 muda, julgadas uma a uma**
  (`scratchpad/c6/muda_b13.py` → `c6/b13_muda.out` e o diff de cada uma em `c6/b13_muda/`; as
  leituras das varreduras com a regra do `2c81e36`). São **66**, contadas pela leitura que o B13
  reordena: as 65 do crítico menos a Levenfis p. 297 (o B13 não forma grupo nela; o que muda a
  página ali é a variante que o árbitro escolhe) e mais a Aagaard *Excelling* pp. 185 e 192 (ruído de
  diagrama). Nenhuma classe nova: **41** só reordenam o ruído que o Tesseract lê dentro de um
  diagrama (a Gallagher pp. 17, 65, 95, 109, 132, 135, 137, 138, 139, 151, 158, 168, 170 e 178, a Estrin pp.
  11, 13, 17, 20, 29, 43, 47, 48, 51, 58, 61, 62, 72, 73, 77 e 78, a Aagaard *Excelling* pp. 180,
  181, 183, 184, 185, 186, 188, 191 e 192, a Karpov 1 p. 395 e a Stean p. 154); **5** são neutras (o
  fólio junto do cabeçalho corrido — a Flores pp. 452, 454 e 458, «452 Training», a Kmoch p. 20 — e
  o colofão da Stefaniu p. 245); **19** melhoram: os índices (a Flores pp. 460, 461, 463 e 464
  0,6708/0,7224/0,7117/0,4960 → 0,0438/0,0853/0,0202/0,0374, a Nunn p. 289 0,3052 → 0,0199, a
  Aagaard *Matter* pp. 892 e 893 0,3688/0,3901 → 0,0729/0,0761, contra as verdades do crítico), as
  listas de lances lidas por linha (a Gallagher pp. 33, 50, 124 e 41 — nesta também «young Spanish»,
  que o Tesseract partira em dois blocos), os sumários com a página ao lado do título (a Levenfis p.
  302, a Stefaniu pp. 242 e 243, a Reinfeld 1977 p. 319) e as linhas partidas que se juntam de novo
  (a Gallagher p. 154, «probably the»; a Gunderam pp. 20, 42 e 46, «Zurück zur Textvariante: 1. d4
  d5»). E **1** piora: a Gallagher p. 175 junta dois lances ao ruído do diagrama numa linha — o
  `7f839f0` a devolve à ordem do livro (item 1.4). **No `7f839f0` são 59**: saem da lista a
  Gallagher p. 175 e seis de ruído que voltam a ser a leitura desligada (a Gallagher pp. 17, 109,
  132, 138 e 178, a Stean p. 154) — 35 de ruído, 5 neutras, 19 melhores, nenhuma pior
  (`muda_b13.py` → `b13_muda_c7.out`, os diffs em `b13_muda_c7/`). Com
  defeito dentro da melhora: a Reinfeld p. 319 e a Stefaniu p. 243 juntam dois títulos numa linha (a
  primeira linha do sumário, e um título sem página). (b) **A Nunn p. 288 da varredura foi julgada errado no
  §0.7**: pelo serviço, na página do PDF, ela sai **0,7516 ligada e desligada** — a leitura do
  RapidOCR, intercalada, que o árbitro escolhe por esse caminho —, e não «para melhor»; e a contagem
  «42 → 0» de linhas com dois verbetes, com a expressão do próprio `cmp_varre_c5.py`, dá 1. Pelo
  importador, a mesma página digitalizada sai 0,0193. O que decide ali é o caminho, e o conserto que
  falta é o do RapidOCR (§0.3). (c) **O «24» do ciclo 4 era 23**: a franja do texto corrido, na
  saída do próprio construtor (`c5/sondas3/notas_risco_w24.out`), soma 23 páginas embaralhadas, e
  não 24. (d) **A Flores p. 461, como ela sai**: aceita, CER 0,0853, com as páginas de três verbetes
  fora do lugar — a continuação de «Flores Rios» («255, 300, 304, 329, 334,» e «412, 428, 434, 444,
  454»), as de «Fressinet» («17, 361») e as de «Friedgood» («4») saem sete linhas abaixo, depois do
  cabeçalho «Name Index», e os dois nomes ficam sem páginas; o §0.7 dizia «agora na ordem do livro».
  (e) **A falha do motor chega a quem revisa**: a região em que um motor falhou leva a falha entre
  os motivos (o item de revisão da Stean p. 165 dizia só «Escore 0.749 abaixo do limite»), e a região
  que o árbitro aceitaria com um motor a menos vai para revisão — a concordância em que ela seria
  aceita tem um motor a menos (`OcrService._with_failures`, `RegionRecognition.engine_failures`); e
  a página cujo OCR levanta (o `MemoryError`) entra na fila de revisão como item de página, abstido,
  com o motivo — antes ela sumia da fila, só nas notas do relatório (`PdfImporter._failed_for_review`).
  Testes com as sabotagens (`test_ocr_service.py`, `test_importer.py`).

- **O que fica, dito** (§0.3): a «Folha transcrita» do Texto empaca a tecla (arquivo de outra
  sessão); a classificação em duas metades com os pontos perdidos pelo OCR, que o B13 intercala
  como a regra do ciclo 5; a coluna de um verbete só, que o `find_gutters` não vê como coluna (a
  primeira linha se junta); uma tabela só de nomes em duas colunas (um emparceiramento) lida por
  colunas, como a regra das faixas comparáveis já fazia; o número inteiro sozinho numa linha («7»)
  tomado por número de lance, que entra numa linha só da tabela; o RapidOCR que intercala colunas
  pelo caminho do serviço (a Nunn p. 288); a lista de lances do Estudo só pelo mouse (não bloqueante
  6: os lances andam pelas setas da sala); a última página de um índice de três colunas
...
- **A regra** (a docstring de `rows.py` tem os números e a razão de cada um): blocos **lado a lado**
  ...
  **Ciclo 4** (§0.6): texto corrido também sem contar palavras (4 linhas ou mais, a mediana acima
  ...
  (`31 Nh1 !?`) é parte dele. **Ciclo 5** (§0.7): a calha antes de toda coluna de índice é da página (as cabeças na margem da coluna em ordem alfabética, ao menos 6), e a calha entre uma partida e o texto também, procurada na faixa ocupada mais perto; e o árbitro compara as leituras na ordem de cada motor — o B13 não decide quem ganha. **Ciclo 6** (§0.8): a regra alfabética sai; a forma de cada faixa, linha a linha (verbetes — um nome e os números dele —, nomes com valores, nomes, números, pontos), e a calha entre duas listas da mesma forma é da página, em qualquer ordem e tamanho (uma faixa de nomes e a das páginas deles contam como uma lista; numa página só de nomes, toda calha entre as colunas); e um grupo nunca tem uma partida e uma coluna de nomes, nem um bloco com uma linha na altura de duas linhas de outro (`7f839f0`: a coluna de colocações que o Tesseract lê como uma linha só).
...
- **Teclado** (`caissa.ui.audit.teclado --pele foco --pdf …Kemeri`, o comando do crítico no ciclo 1,
  ...
  (§0.6). Ciclo 4, no commit: PASSOU a 1280×800 e 1248×640 (Revisão de texto 50/50, Rotulagem 65/65, Galeria 57/57, Resultado 37/37), e a ordem que o portão grava vai da tabela (10) à leitura, às alternativas e à verdade (11–13), às figurinas (14–20) e às seis ações (21–26) — no ciclo 3, as ações eram 11–16 e o cartão 17–26 (`docs/quality/ui/c2_fase5/teclado_foco_20260923_1837*.json`). **Mas essa ordem era a da cadeia, e não a da tecla** (crítico, ciclo 4): o portão andava por `focusNextPrevChild`, e o `QPlainTextEdit` da verdade guarda o Tab. **Ciclo 5:** o campo da verdade deixa o Tab sair, o cartão rola até o foco que entra de fora, e o portão aperta a tecla (`QTest.keyClick`) nos dois sentidos e mede o foco à vista (`visibleRegion`) — no commit, PASSOU nas oito áreas menos o Texto e nos catorze diálogos, nas três peles e a 1280×800, 1248×640 e 1280×641, nos dois sentidos e com o foco à vista; ele achou, antes dos consertos, o comentário, a lista de lances e a caixa de colar do Estudo (tronco `37f662b`, `4f3e218`, `7a78367`) e a «Leitura do motor» da Rotulagem, e acha a «Folha transcrita» do Texto (outra sessão; §0.3). (Mas o portão media a Rotulagem com a tabela vazia, e o Shift+Tab logo depois do Tab, com as rolagens já descidas: o crítico, no ciclo 5, achou a Rotulagem com uma página de linhas presa num laço e o foco a 0 px em quatro áreas do tronco com a rolagem no topo.) **Ciclo 6:** a tabela da Rotulagem deixa o Tab sair e as setas guardam o foco nela, com o Enter levando à verdade; toda rolagem que o Tab alcança de fora mostra inteiro o controle que recebe o foco — na suíte e no tronco (`6e68315`: os modos da aba Livro, a lateral da Galeria, a lista de bases); e o portão mede cada sentido com as rolagens no topo, a Rotulagem com uma página reconhecida pelo serviço do produto (o Kemeri p. 6, 47 linhas, num projeto temporário) e o diálogo de bases também com 40 bases, com as sabotagens `foco` e `tabela` ao lado — no `99ad8d7`, PASSOU nas sete áreas menos o Texto e nos quinze diálogos, nas três peles e três tamanhos, a Rotulagem medida com 47 linhas na tabela (o JSON as grava desde o `99ad8d7`: o do `2c81e36` dizia `null`), e as sabotagens `foco` e `tabela` REPROVAM onde o crítico achou os defeitos (§0.8 item 2).
