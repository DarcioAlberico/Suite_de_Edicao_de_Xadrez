...
| 6 | **Rendimento dos diagramas barrados na exportação** (hachurado e scan-puro) | diagrama | `scan-hachurado`: 21 anotados, 21 casados, **só 10 exportados**; `scan-puro`: 44 casados, 35 exportados — 20 barrados ao todo, exatidão 100 % nos que passam (`F4_FIELD_REPORT.md` §2.1). Causa estrutural documentada em `decode.py` (§2.2): **um tabuleiro reparado pelo decodificador nunca exporta** (`min_confidence ≤ 0,5 < ACCEPT_MIN_CONFIDENCE = 0,80`, `config.py:47`) | um diagrama em dois dos livros antigos hoje não chega ao PGN | médio | `field_exact` sobe sozinho quando `export_rate` cai — publicar os três sempre |
...
| 8 | **Confiança por diagrama calibrada e discriminante** (não por casa) | diagrama | só existe `min_confidence` da casa e o ECE **por casa** (0,000265, `ASSETS.md` §2.14); a temperatura foi rejeitada porque *dobrou a fila sem consertar uma casa* — a fila precisa de um preditor de "este tabuleiro está exato" que **separe** os 20 barrados-certos dos errados | fila de revisão útil; o âmbar passa a significar algo; destrava a alavanca 6 | médio | com 93/94 exatos um preditor constante tem ECE ~0,01 — o portão tem de medir discriminação, não só ECE (§2.4) |
