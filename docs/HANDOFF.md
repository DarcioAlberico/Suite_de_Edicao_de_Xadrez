...
**2026-09-23 (fase 5, ciclo 6 do crítico):** o B13 não usa mais a ordem alfabética: a forma de cada faixa, linha a linha (`rows._band_form`), e a calha entre duas listas da mesma forma é da página (`_list_gutters`); um grupo nunca tem uma partida e uma coluna de nomes (`_names`), nem um bloco com uma linha na altura de duas linhas de outro (`_one_row_high`, `7f839f0`: o Tesseract lê uma coluna de colocações como uma linha só). Para medir uma regra do B13 sem rodar o OCR de novo, as leituras antes do B13 podem ser guardadas (`scratchpad/c6/cache_b13.py`, `cache_corpus.py`) e reordenadas por qualquer versão do `rows.py` (`b13_formas.py`, `todas_leituras.py`). A rolagem que segue o foco (`foco_a_vista.RolagemSegueOFoco`, na suíte e no tronco, `qt/foco_a_vista.py`) ouve `QApplication.focusChanged` e mostra o controle inteiro: toda `QScrollArea` que o Tab alcança de fora a usa. O portão do teclado mede cada sentido com as rolagens no topo e a Rotulagem com uma página reconhecida num projeto temporário (`--sabotar foco|tabela`); o JSON grava as linhas da tabela na medida, e a passada em que a página se reconheceu e a tabela não estava cheia reprova (`99ad8d7`). A região em que um motor falhou vai para revisão com a falha entre os motivos, e a página cujo OCR levanta entra na fila de revisão.
