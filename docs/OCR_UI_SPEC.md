...
| `caissa.ocr.notation.cipher` | resolve dama pela promoção | + `BookCipher` (`notation/book_cipher.py`, **feito** no passo 3): tabela por livro com evidência por linha; provada com 5 provas legais **ou 10 provas visuais** (trocas do leitor de glifos) e contradições ≤ 2 % nas visuais, 0 nas legais; símbolos de um caractere **ou aglomerados** (`'it>`, `l:t`) da camada danificada; nunca uma letra de peça do idioma do livro. O `?` do decodificador virou coringa do reparador (resolvido só quando uma peça é legal) e as atribuições de símbolos são hipóteses de reprodução (`_best_assignment`) |
| `caissa.ingest.pdf.ocr_service` (modelo por livro) | `figurine_candidates`: o modelo do livro só como candidato, nunca âncora | **Executado (passo 4b):** `book_model_anchors` — dentro do livro registrado para o PDF (`livros.json`), `TunedTesseractEngine` sobre a pasta do livro é o âncora do idioma do livro (resultado devolvido com o idioma pedido, `meta["model"]`); fora do livro, candidato como antes. Medido: 193 → 204/204 figurinas na `calib`; registro cruzado inventa 519 figurinas em 6 páginas — o isolamento por impressão digital é a guarda |
| `caissa.ocr.training.tesseract_finetune` | linhas de texto | + linhas negativas e sobreamostragem de classes raras (`FineTuneConfig.negatives`, `.oversample_rare`; `training/negatives.py`). **Executado (passo 4):** um negativo é uma linha de prosa **sem figurina** do próprio livro re-renderizada sob a textura dos controles (foto, ruído, manchas, fax) **com margens só de textura** dos dois lados e a verdade inalterada — o `lstmtraining` pula verdade vazia, então "textura → nada" só se ensina pelas margens; a sobreamostragem repete as linhas da peça com menos linhas que a mediana até uma fração dela |
...
| `caissa.vision.detect.font_catalog` | 37 famílias (29 verificadas, 7 não, `skak_diagram` inferida) | + fallback: glifo vetorial desconhecido renderizado → leitor de glifos → `RecognitionPath.VECTOR_INFERRED` a ≤ 0,85. **Executado (passo 10):** `detect_unknown_font_lattices` (reticulado 8×8 de fonte fora do catálogo, sem torch) + `inferred_font_finder` (recorte das casas → classificador, ≤ 0,85) no `combined_finder`; **e** uma casa ausente na camada de texto deixa de ser "vazia e exata" (Polgar: 61/114 posições erradas, torres em casa escura omitidas pelo extrator): ≤ 0,60, `DiagramHit.holes`, completada pelo classificador só onde a fonte se calou (`fill_holes`) |
...
| `Paragraph` de estilo `Movetext` → `GameScore` | não existe classe `Movetext`: é `ParagraphStyle(name="Movetext")` (`importer.py`) sobre `RegionKind.MOVETEXT` (`paragraphs.py`). Passo novo `caissa.ingest.pdf.games` (T3): FEN do diagrama + lado + tokens reparados → analisador tolerante de `caissa.notation` (`parser`, `book_import`, `variation_builder`, `pipeline.build_games_for_export` — absorvidos do `PGN_Live_Editor`, nunca chamados por `ingest/`) → `legality_repair` → `GameScore` (`core/model/game.py`; variantes são `MoveNode.children[1:]`) com proveniência por lance; o parágrafo permanece quando não há posição de partida. **Executado (passo 11, portão vermelho):** `games.attach_games` no fim de `_to_ir` (`PdfImportOptions.games`); coluna de `Movetext` consecutivos = uma partida; posição = diagrama acima da coluna (geometria) ou posição em que a partida anterior parou; só o tronco vira lance, prosa e variantes ficam em `comment_before/after`; `is_invention` encerra a cadeia quando o reparo muda peça, casa, captura ou promoção do token impresso |
...
| Aba Rotulagem (Qt) e bancada Tk | o revisor escolhe a página a olho; 13 rotuladas de 200 | **«Próxima que vale»** (`caissa.ocr.labeling.queue`, passo 5): pontua uma amostra espaçada das páginas não rotuladas pelo que um rótulo mudaria — linhas `REVIEW`/`ABSTAINED`, linhas duvidosas a meio peso, blocos `movetext` sem `start_fen`, livro sem rótulo, item do livro sem idioma/estrato no manifesto privado; linhas da partição cega não contam — em thread cancelável (segundo clique), lista com o porquê, «Abrir página»; `caissa-rotular --sugerir` imprime a mesma fila sem janela |
| Revisão de texto | inexistente (só bancada) | **`PainelDeRevisaoDeTexto`** sobre `ReviewQueue`: só `REVIEW`/`ABSTAINED`, recorte + leitura + alternativas + motivo, aceitar / editar / manter imagem → IR (U2); reutiliza os widgets de `caissa.ui.views.rotulagem`. **Executado (passo 14):** `caissa/ui/views/revisao_de_texto.py`, aba do tronco `abas.REVISAO_DE_TEXTO`; o cartão comum é `ui/widgets/cartao_da_linha.CartaoDaLinha`; o «→ IR» é por `ReviewDecisions` aplicadas na importação (`PdfImportOptions.review_decisions`), não por edição do IR em memória |
| Fita (pele `Fita`, S-227 = Imagem 2) | **sete grupos** (`ui/comandos.py` `GRUPOS`, decisão registrada); inacabada: título ausente na compacta (0/5), 6/24 sem rótulo, tinta 17,6–41,4 % — números de scripts avulsos, sem portão | os três números fechados **com portão próprio** (`caissa.ui.audit.fita`, `caissa.ui.audit.icones` — novos, promovidos de `benchmarks/reports/ui/c12/`), ícone 24 px tinta ≥ 50 %, rótulo desenhado, hit ≥ 40 px; os sete grupos mantidos (U3). **Executado (passo 12):** 24/24 rótulos (`Comando.rotulo_na_fita`), 5/5 cabeçalhos nas duas densidades (orçamento compacto 72 px), hit ≥ 43 px; portão de ícones reescrito para traço ≥ 2 px / caixa ≥ 60 % / tinta ≥ 10 % — "tinta ≥ 50 %" era silhueta, não traço |
