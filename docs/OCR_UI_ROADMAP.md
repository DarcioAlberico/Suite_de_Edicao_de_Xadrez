...
**Executado em 2026-09-16** (`OCR_UI_REPORT_C1.md` §18): 3 ações, sem zoom; a sabotagem reprova
com 4; os cinco portões da F9 e o `comandos` PASSOU. O `percurso` da tarefa 6 é o
`--fluxo casa` do instrumento que o passo 17 criou. O âmbar por margem entrou sem esperar o
passo 8: ele diz «o modelo hesitou aqui» (e a dica diz entre o quê), e não p(exato).
...
| 2026-09-20 | 0b / 8 | **18 FENs confirmadas pelo usuário e gravadas (tronco `0fbb908`); passo 8 remedido, ainda vermelho** — `OCR_UI_REPORT_C1.md` §19.5 | 114/115 diagramas com placement; negativos 2 → 10. `diagram_confidence_gate`: AUROC 0,899 (< 0,90), ECE 0,011, `min_confidence` sozinho 0,966; sabotagem por ruído 0,206 — **o instrumento passou a distinguir** (em §4 era 0,02 vs 0,12). Com 10 negativos o modelo de dez sinais fica abaixo do sinal cru: população, não sinal. Produto inalterado; faltam os ≥ 20 errados em páginas novas | usuário + construtor |
| 2026-09-20 | 9 / 0b | **tarefa 1 do passo 9 executada; rascunho do 0b entregue** — `OCR_UI_REPORT_C1.md` §19 | `tools/f4_field_failures.py --barrados`: 12 barrados (1 ilegal, 7 reparo, 4 casa fraca) + 8 exportados sem FEN; só 2 dos 12 têm veredito — o diagnóstico confirma o §4: a alavanca é anotar. Leitura visual independente dos 18 casados sem FEN em `docs/quality/0b/propostas_0b.json` (`confirmado: false` em todas; `tools/aplicar_0b.py` grava só as confirmadas): se valerem, os 8 exportados estão certos e **8 dos 10 barrados estão errados** (Koblenz ×4 com 8–11 casas, Niemeijer ×3, Stefaniu) — negativos passam de 2 para 10 em 114, ainda < 30; o 0b continua humano (confirmar as 18 e anotar ≥ 20 errados em páginas novas). Nada gravado no conjunto de campo | construtor |
| 2026-09-16 | 13 | **executado, portão PASSOU (3 ações; sabotagem reprova com 4)** — `OCR_UI_REPORT_C1.md` §18 | Tronco (sobre a fusão `4d8c894`): `ui/recorte_do_diagrama.py` (casa ↔ pixel com o giro, top-3, margem, âmbar por margem, `Tinta`), `qt/painel_de_recorte.py` (o recorte ao lado do tabuleiro, espelho do ponteiro e da seleção, dica das três leituras, teclado, vazio desenhado), `tabuleiro_editavel.pressionar/apontar`, divisor 1:1 sobre pisos iguais no Resultado, «Esconder incerteza» (tinta por margem ligada por padrão), caixas «duvidoso» e «corrigido» (`page_overlay`, `visor`), `editor_model.hand_edited_indices`, janela 1.967 → 1.979. Suíte: `percurso --fluxo casa` (+ `--sabotar sem_sincronia`). Achados: a legenda inteira no rótulo de detalhes forçava a janela a 1.323 px (anterior ao passo; agora uma linha, e a janela cabe em 768 com página lida); empilhar o recorte acima do tabuleiro cria um laço de piso de altura; pisos iguais + folga 1:1 dão os dois quadrados do mesmo tamanho. O âmbar diz «hesitou», não p(exato) — o passo 8 continua atrás de 0b | construtor |
| 2026-09-16 | 17 | **fundido em `4d8c894`** (`git merge --no-ff`, `religa-as-decisoes-orfas`) pelo critério de Q4 — portões e `percurso` verdes em `c20`; o crítico visual (C3) revisa o fundido; desfazer é `git revert -m 1 4d8c894` | construtor |
| 2026-09-16 | 17 | **em ramo próprio (`passo-17-trilho`, commits `10aac70` e `faf0b1a` no tronco); as quatro tarefas executadas, todos os portões PASSOU** — `OCR_UI_REPORT_C1.md` §17 e §17.7 | Q4 = sim (usuário). Suíte: `keep_partial` no importador (cancelar a 30 % devolve 3 de 8 páginas, marcado `canceled`; a sabotagem é o comportamento antigo, afirmado em teste), `ui/trilho.py` (regra do estado por página), `ui/views/importacao.py`, `ui/audit/percurso.py` (novo portão, tudo em pasta temporária — a 1.ª corrida gravou uma amostra real no dataset do tronco, revertida). Tronco: `qt/trilho.py` (miniaturas pelo processo de trabalho, três marcas, primeira duvidosa, importar/cancelar/exportar), `qt/importador_de_livro.py` (ponte + BusyRegistry), 4 comandos, catraca 1.984. `teclado`, `comandos`, `bloqueio` PASSOU com o trilho. **Tarefa 3** (`faf0b1a`): a faixa de abas vira `Livro | Dataset | Galeria | Rotulagem | Revisão de texto`, e Resultado, Estudo, Revisão e Texto são modos da `Livro` (`qt/painel_principal.py`, `qt/areas_de_trabalho.py`; janela 1.984 → 1.967, catraca descida; o estado guarda o modo, sessões antigas reabrem no lugar). Portões em `c20`: `teclado` (a régua do grupo de botões corrigida — `QButtonGroup` sem `autoExclusive` é um ponto de parada com setas, provado em PyQt6), `contraste`, `texto_pintado`, `bloqueio`, `quadros`, `comandos`, `percurso` **todos PASSOU**. **O ramo não se funde** até o crítico visual | construtor |
| 2026-09-16 | 16 | **executado; `contraste`/`teclado`/`texto_pintado` PASSOU, `vazio` REPROVOU (declarado)** — `OCR_UI_REPORT_C1.md` §16 | Q3 = Foco (decisão do usuário): `ui/pele.PADRAO = FOCO`, primeira do menu, as três mantidas (R3.6); a escura é projetada (`NO_CROMO_ESCURO`, conferido). **Achado:** o piso da janela estava em 827 px na Foco (793/743 nas outras) — acima dos 768 da F9-C2 — pela lateral da Galeria (531 px) e pelo cartão da Rotulagem (496); as duas rolam e o piso cai para 674/640/640: a janela cabe em 1366×768 nas três peles. Polimento: `tnum` nos contadores (`PROPRIEDADE_TABULAR`; a Segoe UI já era tabular), contorno neutro na página e no recorte, legenda 3–8 linhas. Sabotagem do contraste (`--sabotar`, 3,90:1) acusa 12 pares do token plantado, 0 na clara. **`vazio` (instrumento novo, régua do crítico C5/C9): 6 de 8 painéis > 200 kpx a 4K antes e depois** — mede área de dados vazia a 100 %, não leiaute; fica vermelho com a leitura em §16.4 e a proposta para o crítico (200 % ou excluir dados vazios). Retratos (96) e pranchas prontos; **o crítico visual às cegas (§11.4) é do crítico** | construtor |
| 2026-09-16 | 15 | **executado, portão PASSOU** — `OCR_UI_REPORT_C1.md` §15 | `bloqueio` 3 × PASSOU (pior operação 6,6 ms; era 170), sabotagem (`--sabotar`) acusa 7; `quadros` zoom 346–415 fps @ p95 (era 75–82). **O passo mudou de forma pela medição:** não foram ladrilhos — uma `QThread` não tira nada da thread da janela quando o que roda nela segura o GIL, e o `get_pixmap` do PyMuPDF (51 ms), o `labels.csv` (55 ms) e a detecção de fundo seguram; faixas de 512 px não ajudam (o scan é decodificado inteiro a cada faixa). Rasterização, CSV e detecção vão para um **processo filho** (`processo_de_trabalho.py`); `sys.setswitchinterval` → 0,1 ms na primeira `Tarefa`; a redução ao zoom roda ao fundo com quadro provisório (`cv2.resize` solta o GIL, `QImage.scaled` não); SQLite da Galeria e contagem de páginas fora da thread; S-123 mantida. Bandeira `painel_do_pdf.RASTERIZAR_AO_FUNDO`. **Portão ajustado:** a operação «rasterizar 300 DPI (`render_pdf_page` direto)» vira referência publicada fora de `viola` — o produto não a chama mais na thread da janela. Tronco `8d9b02f` (sobre `8b61a3e`, o WIP da outra sessão guardado a pedido do usuário). Pendente: reconstruir o bundle e conferir o `spawn` dentro do `.exe` | construtor |
| 2026-09-15 | 15 | **tarefa 0 executada; passo suspenso** — `OCR_UI_REPORT_C1.md` §14 | `quadros` já passa (zoom 75–82 fps @ p95, 3×); `bloqueio` reprova nas 7 operações do C16, 5 delas fora do visor (rasterizar na thread de UI em `painel_do_pdf`, SQLite da Galeria, amostras do Dataset). Os arquivos a mudar (`visor.py`, `painel_do_pdf.py`, `painel_da_galeria.py`, `painel_do_dataset.py`, `viewport.py`) têm 1.267 linhas não commitadas de outra sessão — suspenso até ela commitar; o portão precisa de render e E/S fora da thread, não só de ladrilhos | construtor |
