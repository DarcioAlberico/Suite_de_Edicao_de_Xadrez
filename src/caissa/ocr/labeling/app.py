"""``caissa-rotular`` — a bancada de verificação e treino por livro (Sol §SOL-0/§SOL-11).

O ciclo do FineReader dentro do caissa: abrir um PDF novo, rotular algumas páginas,
treinar, e a importação **desse** PDF passa a ler melhor.
...
``lstmtraining`` consome.  *Treinar…* roda o ajuste fino do Tesseract sobre a verdade
deste livro (``caissa.ocr.training``), grava o modelo em ``models/tessdata/livros/<livro>/``
e o **registra para o PDF** (``livros.json``): a importação e esta janela passam a
consultá-lo como segunda opinião nas regiões de lances.  *Medir no livro…* relê as
páginas rotuladas sem e com o modelo e compara com a verdade — o número que diz se o
treino valeu.

    caissa-rotular [pasta_do_projeto] [--pdf livro.pdf] [--reviewer nome]
    python -m caissa.ocr.labeling.app …
...
class LabelWindow:
    ...
    def _service(self) -> Any:
        """The product's service, pointed at this book's fine-tune when it has one —
        so the hypothesis the reviewer sees is the one the importer would emit."""
    ...
    def _refresh_book(self) -> None:
        """Re-read ``livros.json`` for the current document and drop the service."""
        self.book = None
    ...
    def _book_status(self) -> str:
        if self.book is None:
            return ""
        names = ", ".join(self.book.models)
        return f" · modelo do livro: {names} ({self.book.trained_at[:10]})"
    ...
    def open_training(self) -> None:
        self.save()
        TrainingDialog(self.root, self.project, document=self.document, on_trained=self._trained)

    def _trained(self, book: BookModel | None) -> None:
        """After a fine-tune: the window reads with the new model from now on."""
        self._refresh_book()
        self._refresh_status()
        if book is not None:
            messagebox.showinfo(
                ...
                "A importação deste PDF e o F5 desta janela passam a usá-lo como segunda "
                "opinião nas regiões de lances. «Medir no livro…» diz quanto mudou.",
                parent=self.root,
            )
    ...
    def open_measure(self) -> None:
        ...
        if self.book is None:
            messagebox.showinfo(
                "Medir no livro",
                "Este livro ainda não tem modelo registrado: treine primeiro (Treinar…, "
                "com «modelo deste livro» marcado).",
                parent=self.root,
            )
            return
        MeasureDialog(
...
class TrainingDialog:
    """*Treinar…* — the fine-tune, by default **for the open book**.

    With «modelo deste livro» on, only the book's lines train, the model
    goes to ``models/tessdata/livros/<livro>/`` and the result is registered
    for the PDF's content hash, so the importer and the window use it on
    ...
    def __init__(  # noqa: PLR0915 - one form, laid out row by row
        ...
        *,
        ...
        self.base_lang = tk.StringVar(value=book_lang)
        self.base_model = tk.StringVar(value=self._best_model(book_lang))
        ...
        self.book_check = ttk.Checkbutton(
            ...
            variable=self.for_book,
            command=self._apply_book_choice,
        )
        self.book_check.pack(side=tk.LEFT)
        if document is None:
    ...
    @staticmethod
    def _best_model(lang: str) -> str:
        """The float base already downloaded for ``lang``, or nothing."""
        candidate = DEFAULT_BEST / f"{lang}.traineddata"
    ...
    def _book_label(self) -> str:
        ...
        return (
            ...
            f"{book_dir(DEFAULT_MODELS, self.document)} e usado ao importar este PDF"
        )

    def _apply_book_choice(self) -> None:
        if self.for_book.get() and self.document is not None:
            self.out_var.set(str(book_dir(DEFAULT_MODELS, self.document)))
        elif self.out_var.get() == str(book_dir(DEFAULT_MODELS, self.document or "")):
            self.out_var.set(str(DEFAULT_MODELS))
    ...
    def _config(self) -> FineTuneConfig:
        for_book = bool(self.for_book.get()) and self.document is not None
    ...
    def _finish(self, report: Any) -> None:
        """Register a finished book model and tell the window."""
        book: BookModel | None = None
        if report.status == "trained" and self.for_book.get() and self.document is not None:
            try:
                ...
                book = register_training(
                    ...
                    self.document,
                    report,
                    base_lang=self.base_lang.get().strip(),
                )
                self._append(
...
class MeasureDialog:
    """*Medir no livro…* — the labelled pages read again, without and with
    the book's model, scored against the reviewer's truth."""

    def __init__(
        self, parent: tk.Tk, project: LabelProject, document: str, *, services: dict[str, Any]
    ) -> None:
        self.project = project
        self.document = document
        ...
        self.queue: queue.Queue[tuple[str, Any]] = queue.Queue()
        self.top = tk.Toplevel(parent)
        self.top.title(f"Medir no livro — {document}")
        self.top.geometry("860x560")
        pages = [p.page_index for p in project.pages_of(document) if p.regions]
        bar = ttk.Frame(self.top, padding=8)
        bar.pack(side=tk.TOP, fill=tk.X)
        ttk.Label(
            bar,
            text=f"{len(pages)} página(s) rotulada(s) serão lidas de novo, sem e com o modelo, "
            "e comparadas com a verdade.",
        ).pack(side=tk.LEFT)
        self.run_button = ttk.Button(bar, text="Medir", command=self.run)
        self.run_button.pack(side=tk.RIGHT)
        ttk.Button(bar, text="Cancelar", command=self.cancel).pack(side=tk.RIGHT, padx=6)
        self.progress_label = ttk.Label(self.top, text="", padding=(8, 0))
        self.progress_label.pack(side=tk.TOP, fill=tk.X)
        self.text = tk.Text(self.top, wrap=tk.NONE, font=("Consolas", 10))
        ...
        try:
            while True:
                ...
                if kind == "progress":
                    ...
                    self.run_button.configure(state=tk.NORMAL)
                    self.progress_label.configure(text=f"ERRO: {payload}")
        except queue.Empty:
            pass
        if self.top.winfo_exists():
            self.top.after(100, self._poll)
    ...
    def _show(self, measure: BookMeasure) -> None:
        ...
        self.progress_label.configure(text=f"gravado em {md_path} e {json_path.name}")
        self.text.delete("1.0", tk.END)
        self.text.insert(tk.END, measure.markdown())
